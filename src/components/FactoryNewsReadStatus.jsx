 ...
 * 全工場の既読／未読バッジ一覧
 */
export function FactoryNewsReadStatus({ news, reads, factories, compact = false, comfortable = false }) {
  ...
  const large = comfortable && !compact;
  ...
  return (
    <div className={large ? 'space-y-3' : compact ? 'space-y-1' : 'space-y-2'}>
      <p
        className={
          large
            ? 'text-sm font-black text-slate-600 dark:text-slate-300 sm:text-base'
            : 'text-[10px] font-black uppercase tracking-wide text-slate-500 dark:text-slate-400'
        }
      >
      ...
      <ul
        className={
          ...
          (compact || large ? '' : ' max-h-32 overflow-y-auto')
        }
      >
        {statuses.map((s) => (
          <li
            ...
            className={
              (large
                ? 'rounded-full border-2 px-3.5 py-1.5 text-sm font-bold sm:text-base '
