...
export function CharterBookingCalendar({ bookings = [], loading = false, error = '' }) {
  ...
  // YYYY-MM-DD 同士を比較し、UTC変換による日付ずれを避ける。
        ...
        <div className="mt-1.5 grid grid-cols-7 gap-1">
          {days.map((day) => {
            ...
            return (
              ...
              >
                <p className={isPast ? 'text-xs font-black text-slate-400' : 'text-xs font-black text-slate-500 dark:text-slate-400'}>
                  {d.getDate()}
                </p>
                {list.length > 0 ? (
                  <span
                    className={
                      'mt-1 inline-flex rounded-md px-1.5 py-0.5 text-[10px] font-black ' +
                      (isPast
              ...
              <li
                ...
                className={
                  ...
                  (selectedDateIsPast
                    ...
                    : 'border-indigo-200 bg-indigo-50 dark:border-indigo-700 dark:bg-indigo-950/40')
                }
              >
                <p className={selectedDateIsPast ? 'text-sm font-black text-slate-700 dark:text-slate-300' : 'text-sm font-black text-slate-900 dark:text-slate-100'}>
                  🏭 {b.factoryName}
                </p>
                <p className={selectedDateIsPast ? 'mt-1 text-xs font-bold text-slate-500 dark:text-slate-400' : 'mt-1 text-xs font-bold text-slate-600 dark:text-slate-300'}>
