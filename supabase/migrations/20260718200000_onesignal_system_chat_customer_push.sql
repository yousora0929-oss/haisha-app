-- system チャット（受注・保留・キャンセル・内容変更など）もカスタマー宛てプッシュ対象にする
-- ベース: 20260627000500_onesignal_trigger_multi_tier.sql（現行リモート定義）
  ...
  if tg_op = 'INSERT' then
    ...
    if coalesce(old_status, 'pending') in ('pending', 'pending_association', '')
      ...
      if new_len > old_len and new_len > 0 then
        ...
        if chat_from in ('factory', 'admin', 'system') then
...
comment on function public.trigger_onesignal_order_push() is
  'orders INSERT/UPDATE で OneSignal Edge Function を呼ぶ。system チャットは customer_chat として扱う。';
