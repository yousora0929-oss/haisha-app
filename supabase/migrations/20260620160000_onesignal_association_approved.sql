-- 組合承認（pending_association → pending）時に工場プールへ通知
...
comment on function public.trigger_onesignal_order_push() is
  'orders 変更時に onesignal-push へ URL params で通知（consult/association_approved/status/chat/map/escalation_expanded を判定）';
