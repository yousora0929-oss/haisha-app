-- 配合計画書依頼: 検索用スナップショット（現場名・業者名・現場住所）を追加。
-- RPC シグネチャは変更しない。p_request の追加キーのみ読む。
...
comment on column public.mix_design_requests.project_name is
  '工事名（現場名）の依頼時点スナップショット。検索・印刷用。projects.name は更新しない。';
comment on column public.mix_design_requests.contractor_name is
  '業者名の依頼時点スナップショット。prime_contractor_name（元請宛名）とは別。';
comment on column public.mix_design_requests.site_address is
  '現場住所の依頼時点スナップショット。';
